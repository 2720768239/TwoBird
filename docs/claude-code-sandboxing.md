...
![Claude Code's sandboxing architecture isolates code execution with filesystem and network controls, automatically allowing safe operations, blocking malicious ones, and asking permission only when needed.](https://aka.doubaocdn.com/s/7kPg1wZqTO)
...
![Claude Code's Git integration routes commands through a secure proxy that validates authentication tokens, branch names, and repository destinations—allowing safe version control workflows while preventing unauthorized pushes.](https://aka.doubaocdn.com/s/VSxt1wZqTO)
...
2. Go to [claude.com/code](http://claude.ai/redirect/website.v1.a813babd-04bf-434c-bb7f-49aded8c6b65/code) to try out Claude Code on the web.
